		...
		private function writeRow():void
		{
			...
			if (!transparent)
			{
				for (x = 0; x < width; x++)
				{
					...
					IDAT.writeUnsignedInt(uint(((pixel & 0xFFFFFF) << <img src="http://www.bit-101.com/blog/wp-includes/images/smilies/icon_cool.gif" alt="8)" class="wp-smiley"> | 0xFF));
				}
			}
			else
			{
				for (x = 0; x < width; x++)
				{
						...
						IDAT.writeUnsignedInt(uint(((pixel & 0xFFFFFF) << <img src="http://www.bit-101.com/blog/wp-includes/images/smilies/icon_cool.gif" alt="8)" class="wp-smiley"> |
							(pixel >>> 24)));
				}
			}
